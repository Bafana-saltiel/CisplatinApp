...
# Updated with monotherapy-response and validation layers
# ============================================================
...
simulate_reference_combo <- function(model_id, combo_name, therapeutic_class, drug_names, doses, noise_sd) {
  ...
  E_observed <- pmax(pmin(E_bliss + rnorm(length(E_bliss), mean = 0, sd = noise_sd), 100), 0)
  ...
  bind_cols(
    ...
    dose_grid,
    inhibition_matrix,
    tibble(BlissExpected = E_bliss, Observed = E_observed, BlissScore = E_observed - E_bliss)
  )
}
...
simulate_reference_plus_partner <- function(model_id, combo_name, therapeutic_class, drug_names, partner, doses, noise_sd) {
  df_model <- Cervix_GDSC_All %>% filter(ModelID == model_id)
...
run_reference_pipeline <- function(combo_name, drug_names, compound_ids, therapeutic_class, doses, noise_sd, selected_partner_ids = NULL) {
  ...
  if (nrow(reference_bliss) == 0) return(empty_result())
  ...
  reference_summary <- reference_bliss %>%
    ...
    summarise(
      ReferenceMeanBliss = safe_mean(BlissScore),
      ReferenceMaxBliss = safe_max(BlissScore),
      ReferenceMinBliss = safe_min(BlissScore),
      ReferenceSDBliss = safe_sd(BlissScore),
      ReferenceMeanExpected = safe_mean(BlissExpected),
  ...
  run_model <- function(model_id) {
    ...
    if (nrow(partners) == 0) return(list(monotherapy = tibble(), third_bliss = tibble()))
  ...
  third_ranking <- third_bliss %>%
    ...
    summarise(
      ...
      MeanBliss = safe_mean(BlissScore),
      MaxBliss = safe_max(BlissScore),
      MinBliss = safe_min(BlissScore),
      SDBliss = safe_sd(BlissScore),
      MeanExpected = safe_mean(BlissExpected),
      MeanObserved = safe_mean(Observed),
      .groups = "drop"
    ) %>%
    left_join(reference_summary %>% select(ModelID, ReferenceMeanBliss), by = "ModelID") %>%
    left_join(
      monotherapy_summary %>%
        select(
    ...
    mutate(
      ...
      PharmacologicalInterpretation = case_when(
        MeanBliss > ReferenceMeanBliss & MeanMonotherapyResponse < 40 ~ "Combination-specific candidate",
        MeanBliss > ReferenceMeanBliss & MeanMonotherapyResponse >= 75 ~ "Strong monotherapy with additional combination benefit",
        MeanBliss > ReferenceMeanBliss & MeanMonotherapyResponse >= 40 ~ "Moderate monotherapy with combination benefit",
        MeanBliss <= ReferenceMeanBliss & MeanMonotherapyResponse >= 75 ~ "Strong monotherapy but limited combination advantage",
        MeanBliss < 0 & MeanMonotherapyResponse < 40 ~ "Low monotherapy response and antagonistic combination",
...
# ============================================================
# 5. AVAILABLE THIRD DRUGS
# ============================================================

Available_Third_Drugs <- Cervix_GDSC_All %>%
  distinct(CompoundID, CompoundName) %>%
  left_join(
    PortalCompounds %>% select(CompoundID, Synonyms, GeneSymbolOfTargets, TargetOrMechanism),
    by = "CompoundID"
  ) %>%
...
Third_Drug_Choices <- setNames(
  ...
  paste0(Available_Third_Drugs$CompoundName, " [", Available_Third_Drugs$CompoundID, "]")
)
...
server <- function(input, output, session) {
  ...
  results <- eventReactive(input$run, {
    ...
    if (input$third_drug_mode == "selected") {
      validate(need(nrow(selected_drugs) > 0, "Please select at least one third drug."))
    ...
    withProgress(message = "Running pharmacology and validation layers...", value = 0, {
      incProgress(0.20, detail = "Running reference-regimen Bliss analysis")
  ...
  output$top10_plot <- renderPlot({
    ...
    ggplot(plot_data, aes(x = reorder(ThirdDrug, MeanBliss), y = MeanBliss, fill = Interaction)) +
      geom_col() +
      geom_hline(
      ...
      theme_bw(base_size = 11) +
      labs(
        title = paste0("Cervical models: ", str_replace_all(input$combo, "_", " "), " + top third drugs"),
